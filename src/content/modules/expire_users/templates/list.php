    ...
    <table class="tablesorter">
        ...
        <tbody>
            <?php foreach (ViewBag::get("users") as $user) { ?>
                <tr>
                    <td class="hide-on-mobile"><?php echo $user->getId(); ?></th>
                    <td><?php Template::escape($user->getUsername()); ?></th>
                    <td class="hide-on-mobile"><?php Template::escape($user->getLastname()); ?></th>
                    <td class="hide-on-mobile"><?php Template::escape($user->getFirstname()); ?></th>
                    <td class="hide-on-mobile"><?php echo $user->isLocked() ? translate("yes") : translate("no"); ?></td>
                    <?php
                    $cssClass = "";
                    $expire_date = UserSettings::get("expire_date", "int", $user->getId());
                    if ($expire_date && time() > $expire_date) {
                        $cssClass = "text-red";
                    }
                    ?>		
